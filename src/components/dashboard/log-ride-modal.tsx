...
const DEFAULT_TRIGGER_CLASS_NAME =
  "inline-flex items-center gap-2 rounded-sm px-4 py-2 text-sm font-semibold uppercase tracking-[0.1em] text-[#FBF7EC] transition hover:bg-[#A92E22]";
...
export function LogRideModal({
  ...
  const [selectedCoaster, setSelectedCoaster] =
    useState<CoasterSearchResult | null>(initialCoaster ?? null);
            ...
            <form onSubmit={handleSubmit} className="space-y-4 px-5 py-5">
                ...
                {selectedCoaster ? (
                  ...
                  <div className="relative">
                    ...
                    {searchTerm.trim().length >= MIN_QUERY_LENGTH && (
                      ...
                      >
                        {isSearching && (
                          <li className="px-3 py-2 text-sm text-[#8B8368]">
                            Searching…
                          </li>
                        )}
              ...
              <div>
                <label htmlFor="notes" className={labelClass}>
                  Notes{" "}
                  <span className="normal-case text-[#8B8368]">(optional)</span>
                </label>
                <textarea
                  id="notes"
                  value={notes}
